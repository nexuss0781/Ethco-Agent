...
export async function executeWorkspaceTool(name: string, args: Record<string, any>): Promise<any> {
  ...
  // Normalize aliases (deprecated duplicate tool names map to their canonical tool)
  const canonicalName = {
    run_command: "bash",
    view_file: "read",
    create_file: "write",
  ...
  if (FS_TOOL_NAMES.has(canonicalName)) {
    return executeFsTool(canonicalName, args);
  }
...
function getSystemPrompt(model?: string, modelId?: string): string {
  ...
  if (isLiteModel) {
    try {
      if (fs.existsSync(litePromptPath)) {
...
app.get("/api/tools", (req, res) => {
  res.json({ tools: ALL_TOOL_DECLARATIONS });
});
...
// 1. GitHub Login — always routed through central Nexuss Auth (no direct GitHub OAuth app)
app.get("/api/github/login", (req, res) => {
  ...
  const nexussRedirectUri = configuredRedirect ? configuredRedirect : `${origin}/api/auth/callback`;
...
// 2. Get GitHub Repository Authorization URL (Nexuss Auth controlled)
...
// GitHub authorization is fully controlled by Nexuss Auth — the OAuth handoff is
// finalized by /api/auth/callback, which stores the github_grant_token and GitHub profile.
...
async function executeOmniRouteTurn(
  ...
  const openAiTools = convertToOpenAiTools(ALL_TOOL_DECLARATIONS);
