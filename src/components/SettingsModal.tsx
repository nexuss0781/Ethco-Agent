...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div
        id="settings-modal"
        className="relative w-full max-w-lg bg-canvas-deep border border-line-soft rounded-2xl shadow-2xl p-6 text-left animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="space-y-5">
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-canvas-deep border border-line-soft space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-line-soft">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-fg-muted flex items-center gap-1.5">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
              ...
              <div className="relative shrink-0">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt={realName}
                    className="w-16 h-16 rounded-2xl border-2 border-teal/80 object-cover shadow-md bg-canvas"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-2xl bg-surface border-2 border-teal/80 flex items-center justify-center text-teal text-xl font-bold shadow-md">
                    {realName.charAt(0).toUpperCase()}
                  </div>
                )}
                <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-canvas-deep border border-line-soft flex items-center justify-center">
              ...
              <div className="space-y-2 flex-1 text-center sm:text-left min-w-0">
                <div>
                  <div className="text-base font-bold text-fg tracking-tight truncate">
                    {realName}
                  </div>
                  <div className="flex items-center justify-center sm:justify-start gap-1 text-xs text-teal font-mono mt-0.5">
                    <AtSign className="w-3 h-3 text-teal" />
                    <span className="font-medium">{username}</span>
                  </div>
                </div>

                {/* Authenticated Gmail (Intact Authentication) */}
                <div className="p-2.5 rounded-xl bg-surface border border-line-soft flex items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="w-6 h-6 rounded-lg bg-surface-2 border border-line flex items-center justify-center shrink-0">
                      <Mail className="w-3.5 h-3.5 text-teal" />
                    </div>
                    <div className="truncate">
                      <div className="text-[10px] text-fg-muted uppercase font-semibold">Authentication Gmail</div>
                      <div className="text-xs font-medium text-fg truncate font-mono">{authGmail}</div>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-surface text-emerald-400 border border-emerald-500/20 shrink-0">
                    Verified
                  </span>
                </div>
              </div>
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-canvas-deep border border-line-soft space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-line-soft">
              <span className="text-[11px] font-semibold uppercase tracking-wider text-fg-muted flex items-center gap-1.5">
                <Github className="w-3.5 h-3.5 text-teal" />
            ...
            {loadingStatus && !ghUser ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3.5 p-3 rounded-xl bg-surface border border-line-soft">
                  {ghUser.avatar_url ? (
                    <img
                    ...
                    <div className="w-11 h-11 rounded-xl bg-surface-2 border border-line flex items-center justify-center text-teal font-bold">
                      GH
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold text-xs text-fg truncate">
              ...
              <div className="flex flex-col items-center text-center space-y-3 py-3">
                <div className="w-12 h-12 rounded-xl bg-surface border border-line flex items-center justify-center">
                  <Github className="w-6 h-6 text-fg" />
                </div>
                <div>
                  <h4 className="text-xs font-semibold text-fg">Connect GitHub Account</h4>
                  <p className="text-[11px] text-fg-muted max-w-xs mt-0.5">
                ...
                <a
                  ...
                  href={GitHubService.getLoginUrl()}
                  className="w-full py-2.5 px-4 rounded-xl text-xs font-semibold bg-brand hover:bg-brand-strong text-white transition-all flex items-center justify-center gap-2 shadow-xs cursor-pointer"
                >
          ...
          {/* Footer Sign Out */}
          <div className="pt-2 flex items-center justify-between text-xs text-fg-muted px-1 border-t border-line-soft">
            <span className="truncate max-w-[280px]">Session authenticated via {authGmail}</span>
            <button
              onClick={() => logout()}
              className="text-fg-muted hover:text-red-400 transition-colors cursor-pointer text-xs font-medium flex items-center gap-1"
            >
